...
function showResultPage(event) {
  ...
  questionsAndResults.innerHTML = `
    <section class="flex flex-col items-center px-4 py-6 text-center bg-green-600 border rounded-lg shadow">
      <p class="text-4xl font-medium text-gray-900 mb-4">${correctAnswersCounter===4?'¡GANASTE!': '¡PERDISTE!'}</p>
      
      <p class="text-md font-medium text-gray-900 mb-4">Respondiste ${correctAnswersCounter} de ${questions.length}</p>

      <p class="text-md font-medium text-gray-900 mb-4">Y este es tu puntaje: ${correctAnswersCounter*(20/questions.length)}</p>

     
      ${correctAnswersCounter===4?'<img class="flex items-center justify-center" src="./ganaste.png" width="300" height="100" />': '    <img class="flex items-center justify-center" src="./perdiste.webp" width="300" height="100" />'}
